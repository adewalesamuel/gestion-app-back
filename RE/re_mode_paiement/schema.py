...
class REModePaiementSchema(SQLAlchemySchema):
    ...
    frais_pourcentage = auto_field(validate=validate.Length(min=1))
    delai_jours = auto_field(validate=validate.Length(min=1))
    actif = auto_field(validate=validate.Length(min=1))
