...
class RCEnginFlottantSchema(SQLAlchemySchema):
    ...
    tonnage_brut = auto_field(validate=validate.Range(min=1))
    longueur = auto_field(validate=validate.Range(min=1))
    annee_construction = auto_field()
    capacite_passagers = auto_field(validate=validate.Range(min=1))
    capacite_fret = auto_field(validate=validate.Range(min=1))
